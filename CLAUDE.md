...
- Build: `colcon build --packages-select chess_ai chess_ai_interfaces`
- Source: `source install/setup.bash`
- Entry points (`chess_ai` 패키지):
  - `ros2 run chess_ai main` — 통합 실행 (`chess_ai/main.py`)
  - `ros2 run chess_ai stockfish` — 체스 엔진 노드 (`chess_ai/stockfish.py`)
  - `ros2 run chess_ai robotaction` — 로봇 동작 노드 (`chess_ai/robot_action.py`)
  - `ros2 run chess_ai object` — 비전 인식 노드 (`chess_ai/vision_db.py`)
- Configs: `src/chess_ai/config/*.json`, `src/chess_ai/chess_ai/data.json`
- Models: `src/chess_ai/models/`
- Interfaces: `src/chess_ai_interfaces/{action,srv}/`
...
- 주의: 현 `setup.py`는 `share/chess_ai/.env`를 데이터 파일로 등록. `.env`가 없으면
  빌드 실패 — 신규 환경에서는 `.env.example`을 복사해 `src/chess_ai/.env` 생성 필요.
